...
if(isset($_POST['sent'])){
    $message=$_POST['text'];
  $a="INSERT INTO `message`(`id`, `message`) VALUES (1,'$message')";
  mysqli_query($con,$a);
$mh="UPDATE `message` SET `message`='$message' where 1";
$result=mysqli_query($con,$mh);
if($result){
    header("Location:message.php");
}}
$select="SELECT  * FROM `message` WHERE id=1";
$res=mysqli_query($con,$select);
$rom= mysqli_fetch_assoc($res);
$message=$rom['message'];
...
<form  method="post" action="message.php">
    <textarea placeholder="Write your message here" rows="20" 
    name="text" id="comment_text" cols="30" 
    class="ui-autocomplete-input" autocomplete="off" 
    role="textbox" aria-autocomplete="list" aria-haspopup="true"></textarea>
    <div style=" font-size:20px; width:100px; height: 10px;margin:auto; float:right; margin-right:310px;">Last message</div>
  <div style="backdrop-filter: blur(10px);position:relative;bottom:377px;	 border-left:2px solid #3498db;border-right:2px solid #3498db;border-top:2px solid #3498db; width: 500px; height:500px; margin:auto; float:right; margin-right:100px;">

<?php
if($message){
    echo $message;
}else{echo "No message yet!";} ?> 
</div>
<input class="sub" name="sent" type="submit" value="Sent">
